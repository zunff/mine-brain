...
- `tests/e2e/` 只放**数据无关**的结构回归：不依赖本机数据库里的特定历史会话，换一台空库机器也能跑（如断言布局/文案形态、零 console 报错、主题持久化）。
- 硬编码本机历史数据（特定记忆数、trace 数、会话标题等）的脚本是**本机探针**，放 `tests/e2e/local/`（已 gitignore），绝不提交——它们不构成可移植回归。
- 新增浏览器脚本先自问：换一台空库机器能通过吗？不能 → `tests/e2e/local/`。
- 本机探针与可移植回归不要混在同一个目录，避免 CI/换机误跑。
